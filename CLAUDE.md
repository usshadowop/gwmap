...
- [`docs/form/outreach-email-intro-template.md`](docs/form/outreach-email-intro-template.md) — optional first-contact ("hello") email sent *before* verification: introduces Jon + the project in three short paragraphs and asks if it's the right inbox for the store (with a "better contact?" reply option — no removal/opt-out, since the listing is fan-sourced and stays either way). No form/button (keeps the first touch from looking like phishing). Marker `Intro email sent on <date>`.
