...
- [`docs/form/outreach-email-intro-template.md`](docs/form/outreach-email-intro-template.md) — optional first-contact ("hello") email sent *before* verification: introduces Jon + the project in three short paragraphs and asks if it's the right inbox for the store. No form/button (keeps the first touch from looking like phishing); offers a "better contact / remove me" opt-out. Marker `Intro email sent on <date>`.
