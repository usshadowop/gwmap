...
- **Update merge preserves never-collected fields.** On an UPDATE to an existing store, `lat`, `lng`, `website`, `phone`, `preorderUrl`, and `note` are preserved so a re-submission can't wipe the map pin / contact info. Blank answers *do* still clear other collected text fields — that was the deliberate choice ("keep coords only, overwrite rest"). **Exception:** `mapsUrl` is now collected via its own optional question, so it's preserved *only when the submission leaves it blank* — a provided link overwrites, a blank one keeps the existing verified URL.
...
- **First real end-to-end test submission still recommended.** The field mapping keys off the question titles in the form (now via tolerant substring matching for the address / Maps-link fields, exact-title for the rest), so it warrants a live pass: submit once through each discount branch (15%, 10%, an off-grid % like 20 to confirm the snap to `15`, loyalty, and none) and confirm each opens a PR with the correct derived `category`, Yes/No booleans, and `address` / `mapsUrl` landing in their now-separate fields. Also test an update that leaves the Maps-link question blank, to confirm an existing `mapsUrl` is preserved (not wiped).
